...
rtclient.Authorizer = function(options) {
  ...
  this.access_token = rtclient.getOption(options, 'access_token');
...
rtclient.Authorizer.prototype.authorize = function(onAuthComplete) {
  ...
  this.fetchUserId(onAuthComplete);
}
...
rtclient.pullrealtimefiles = function(title, callback) {
  gapi.client.load('drive', 'v2', function() {
  	var query_string = "title contains '"+title+"' and trashed = false";
    gapi.client.drive.files.list({q:query_string}).execute(callback);
  });
}
...
rtclient.RealtimeLoader.prototype.load = function() {
  ...
  // Create custom collatborative objects
  // gapi.drive.realtime.custom.registerType(WhiteBoard, 'WhiteBoard');
  // WhiteBoard.prototype.title = gapi.drive.realtime.custom.collaborativeField('title');
...
rtclient.RealtimeLoader.prototype.createNewFileAndRedirect = function() {
  // Check to see if the file previously exists, if not then create the file.
  ...
  var _this = this;
  rtclient.pullrealtimefiles(this.defaultTitle, function(file_list) {
  	// Check to see if there is a file that already exists. If so load the existing file. 
    if (file_list.items != undefined) {
      gapi.drive.realtime.load(file_list.items[0].id, _this.onFileLoaded, _this.initializeModel, handleErrors);
    }
    else {
      // If not create new file
      rtclient.createRealtimeFile(_this.defaultTitle, function(file) {
		if (file.id) {
      	  gapi.drive.realtime.load(file.id, _this.onFileLoaded, _this.initializeModel, handleErrors);
		}
